...
public class BoardController {
    ...
    public ResponseEntity getBoardList(@PageableDefault(page = 0, size = 10, sort = "seq", direction = Sort.Direction.DESC) Pageable pageable) throws Exception {
        List<BoardResult> boardList = boardService.getBoardList(pageable);
        ...
        List<EntityModel> collect = boardList.stream()
                                        .map(board -> EntityModel.of(board,
                                                ...
                                                getLinkAddress().slash(board.getSeq()).withRel("edit")))
